...
print("Dati csv failā ir rindā (raksti '1') vai kolonā(raksti '2')")
rind_colon = input()
if rind_colon == "1":
...
#4*len(data) + 29
ws['A' + str(4*len(data) + 29)] = "Rezultāti"
ws['A' + str(4*len(data) + 30)] = "Metode"
ws['B' + str(4*len(data) + 30)] = "Modelis"
ws['C' + str(4*len(data) + 30)] = "Kļūda"
ws['D' + str(4*len(data) + 30)] = "Prognoze"

ws['A' + str(4*len(data) + 31)] = "Slidosas vidējas metode"
ws['B' + str(4*len(data) + 31)] = str(ws[Xlsx_Letter(3+int(slid_min_index)) + "2"].value)
ws['C' + str(4*len(data) + 31)] = slid_data_klud[1][slid_min_index]
min_klud.append(slid_data_klud[1][slid_min_index])
ws['D' + str(4*len(data) + 31)] = slid_data[1][slid_min_index]
best_prognoz.append(slid_data[1][slid_min_index])

ws['A' + str(4*len(data) + 32)] = "Eksponenciālas noguldīsānas metode"
ws['B' + str(4*len(data) + 32)] = str(ws[Xlsx_Letter(3+int(expon_min_index)) + str(len(data) + 9)].value)
ws['C' + str(4*len(data) + 32)] = ekspon_data_klud[1][expon_min_index]
min_klud.append(ekspon_data_klud[1][expon_min_index])
ws['D' + str(4*len(data) + 32)] = ekspon_data[1][expon_min_index]
best_prognoz.append(ekspon_data[1][expon_min_index])

ws['A' + str(4*len(data) + 33)] = "Tendences metode"
ws['B' + str(4*len(data) + 33)] = str(ws[Xlsx_Letter(4+int(trnd_min_index)) + str(2*len(data) + 16)].value)
ws['C' + str(4*len(data) + 33)] = trend_data_klud[1][trnd_min_index]
min_klud.append(trend_data_klud[1][trnd_min_index])
ws['D' + str(4*len(data) + 33)] = trend_data[1][trnd_min_index]
best_prognoz.append(trend_data[1][trnd_min_index])

if Sezon_papild == "1" or Sezon_papild == "2":
    ws['A' + str(4*len(data) + 34)] = "Sezonālais modelis"
    ws['B' + str(4*len(data) + 34)] = "Sezonālais modelis"
    ws['C' + str(4*len(data) + 34)] = sezon_data_klud[1][0]
    min_klud.append(sezon_data_klud[1][0])
    ws['D' + str(4*len(data) + 34)] = sezon_data[1]
...
if if_save == "yes":
    print("Ievadiet faila nosaukumu (bez .xlsx)")
    name = input()
